...
- 正式订阅 `icons.json` 包含全部 778 个图标；旧订阅 `Semporia.json` 已在 v1.1 移除。
...
- 本仓库 `Universal/IBKR.png` 线上地址返回 HTTP 200；含该地址的最小 Surge 配置通过 Surge 自带检查；正在运行的 Surge 核心也成功获取了这张图片，状态为 200、大小为 14,696 字节。
